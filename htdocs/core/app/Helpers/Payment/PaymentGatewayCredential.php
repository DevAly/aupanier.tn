...
class PaymentGatewayCredential
{
    ...
    public static function get_paymee_credential()
    {
        $paymee_credential_from_database = PaymentGateway::where('name', 'paymee')->first();
        $decoded = json_decode($paymee_credential_from_database->credentials);
        $paymee = new Paymee();
        if($paymee_credential_from_database->test_mode){
            $paymee->setAccountNumber($decoded->test_account_number ?? '');
            $paymee->setApiToken($decoded->test_api_token ?? '');
            $paymee->setIsTestMode($paymee_credential_from_database->test_mode ?? '');
        }else{
            $paymee->setAccountNumber($decoded->account_number ?? '');
            $paymee->setApiToken($decoded->api_token ?? '');
            $paymee->setIsTestMode($paymee_credential_from_database->test_mode ?? '');
        }
