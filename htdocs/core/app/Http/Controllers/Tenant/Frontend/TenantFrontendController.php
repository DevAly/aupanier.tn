...
class TenantFrontendController extends Controller
{
    ...
    {
        $response = checkAndLoginUsingToken('/');
        if($response){
            return $response;
        }
    ...
    public function render_reviews(Request $request)
    {
        ...
        $review_markup = themeView('shop.product_details.markup_for_controller.product_reviews', compact('reviews'))->render();
    ...
    {
        if(!Auth::check()){
            return response()->json(['type' => 'error', 'msg' => 'You are not authenticated']);
        }
...
// You can change the layout, design, and name of the website. Also, order management, blog settings, tax manage, mobile app management, location settings, etc.
   ...
   -------------------------- */
    ...
    public function expired_package_redirection()
    {
        ...
        return themeView('pages.expired');
    }


    public function website_creation_in_progress_redirection()
    {
        return view('tenant.frontend.pages.creationInProgress');
    }
