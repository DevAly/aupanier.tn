...
class LandlordFrontendController extends Controller
{
    ...
    public function check_verify_user_email(Request $request)
    {
        ...
        $buyer = Wallet::where('user_id', $user_info->id)->first();
        if (empty($buyer) && !empty($user_info->affiliation_code)) {
            Wallet::create([
...
//        return redirect()->route('landlord.frontend.plan.order', ['id' => PricePlan::query()->where('package_badge', 'Gratuit')->first()->id]);
        return redirect()->route('landlord.dynamic.page', ['page' => 'pricing-plan']);
    }
    ...
    public function affiliateByCode($code)
    {
        $affilateUser = AffiliateUser::query()->where('code', $code)->where('is_active', 1)->first();
        if($affilateUser){
    ...
    public function ajax_login(Request $request)
    {
        ...
        if (Auth::guard('web')->attempt(['email' => $request->username, 'password' => $request->password], $request->get('remember'))) {
...
//            'username' => ['required', 'string', 'max:191', 'unique:users'],
        ...
        $user_id = DB::table('users')->insertGetId([
            ...
            'city' => $request['city'],
            'username' => Str::slug($request['email'], '_'),
            'password' => Hash::make($request['password']),
            'mobile' => $request['mobile'],
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now(),
            'affiliation_code' => session('affiliate_code'),
            'email_verified' => 1,
        ]);
        ...
        try {
            ...
            Log::channel('slack-new-signup')->info("New Signup: {$user->email} phone: tel:{$user->mobile}");
			$response = Http::get('https://script.google.com/macros/s/AKfycbxgjERTxBE4l8R-kXkdj4PgiX4XpwKK9pD3VSnEh-1euucGEJPwcMdjP5ji98YpwWEw_w/exec?gid=0&name='.$user->name.'&email='.$user->email.'&phone='.$user->mobile);
    ...
    public function newsletter_store(Request $request)
    {
        ...
        return response()->json([
            'msg' => __('Merci de vous inscrire à notre lettre d\'information'),
            'type' => 'success'
        ]);
    }
    ...
    public function user_trial_account(Request $request)
    {
        ...
        if(!is_null($domain_details)){
            ...
            return response()->json([
                'url' => $url.'?aupanier_login_token='.getLoginToken($subdomain),
                'type' => 'success'
            ]);
        }
