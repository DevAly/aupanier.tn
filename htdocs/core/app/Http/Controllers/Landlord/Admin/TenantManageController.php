    ...
    public function resend_verify_mail(Request $request){
        ...
        if (empty($subscriber_details->email_verify_token)){
            ...
            $subscriber_details->save();
             ...
             $message = __('Verification Code: ').'<strong>'.$token.'</strong>'.'<br>'.__('Verify your email to get all news from '). get_static_option('site_title') . '<div class="btn-wrap"> <a class="anchor-btn" href="' . route('landlord.user.login') . '">' . __('Login') . '</a></div>';
