...
//    VerifyUserMailSend::sendMail(\App\Models\User::find(14));
//    die('amine');
//});
...
Route::middleware(['maintenance_mode','landlord_glvar'])->controller(PaymentLogController::class)->name('landlord.')->group(function () {
    Route::get('/paymee-ipn/{id?}', 'paymee_ipn')->name('frontend.paymee.ipn');
...
Route::middleware(['landlord_glvar','maintenance_mode', 'set_lang'])->controller(\App\Http\Controllers\Landlord\Frontend\LandlordFrontendController::class)->name('landlord.')->group(function () {
	...
	Route::get('/from/{code}', 'affiliateByCode')->name('affiliate.by.code');
