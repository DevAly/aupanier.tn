...
@section('scripts')
    ...
    <script>
        if(typeof fbq === "function") {
            @php
                $tndToUsdExchange = tenancy()->central(function ($tenant) {
                    return get_static_option('site_tnd_to_usd_exchange_rate');
                });
            @endphp
            fbq('track', 'Purchase', {
                value: '{{ $payment_details->total_amount / $tndToUsdExchange }}',
                currency: 'USD',
            });
        }
    </script>
