<div class="form-group">
    ...
    <select name="site_global_currency" class="form-control"
        ...
        <?php
        $currencies = ['TND' => 'TND'];
        $currencies = array_merge($currencies, script_currency_list());

        ?>
  @foreach($currencies as $cur => $symbol)
  <option value="{{$cur}}"
...
<div class="form-group">
    ...
    <select name="site_default_payment_gateway" class="form-control">
        @php
            $all_gateways = ['paymee','paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','midtranse','marcedopago','instamojo','cashfree', 'toyyibpay', 'zitopay', 'squareup', 'cinetpay', 'paytabs', 'billplz', 'payfast'];
        @endphp
