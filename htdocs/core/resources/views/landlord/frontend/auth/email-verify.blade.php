            ...
            <div class="row justify-content-center">
               <div class="col-lg-6">
                   <div class="contact-form-wrapper wrapper-verify">
                       <h2 class="title" style="text-align:center">{{__("Verify your email")}}</h2>
...
Le code a été envoyé à : <b>{{ auth()->user()->email }}</b>.<br> En cas d'absence, veuillez vérifier le Spam.<br> Merci pour votre attention.</p>
                               ...
                               <div class="extra-wrap margin-top-20">
                                   <span>{{__('Did not receive the code?')}} <a href="{{route('landlord.user.email.verify.resend')}}" class="color-one">{{__('Resend code')}}</a></span>
                               </div>
                           </form>
                       </div>
                   </div>
