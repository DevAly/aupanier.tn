...
class PageSeed extends Seeder
{
    public function run()
    {
        ...
        update_static_option('home_page', $page_data->id);
        ...
        update_static_option('blog_page', $page_data_2->id);
        ...
        update_static_option('shop_page', $page_data_6->id);
        ...
        update_static_option('blog_page', $page_data_7->id);
    ...
    private function upload_layout($file, $page_id)
    {
        DB::beginTransaction();
        try {
            $file_contents = json_decode(file_get_contents(base_path().'/../core/resources/views/themes/'.$file));
            ...
            if (current($file_contents)->addon_page_type == 'dynamic_page')
            {
                foreach ($file_contents as $key => $content)
                {
                    ...
                    {
						 if($content->addon_name=='ContactAreaOne'){
							if($key2 == 'addon_settings'){
								$addonSettings = json_decode($con);
								...
								$addonSettings->contact_tenant_repeater->repeater_sub_info_[0] = $phone;
								$addonSettings->contact_tenant_repeater->repeater_sub_info_[1] = $email;
								$addonSettings->contact_tenant_repeater->repeater_sub_info_[2] = "";
								$con = json_encode($addonSettings);
							}
						 }
                        $contentArr[$key][$key2] = $con;
						// Log::channel('slack-new-shops')->info("DEBUG: {$key} == {$key2}".json_encode($con));
                    }
                    if($content->addon_name=='ContactAreaOne'){
                        $page_contact=json_decode($content->addon_settings);
                        $info=$page_contact->contact_tenant_repeater->repeater_sub_info_;
                        $phone = tenancy()->central(function ($tenant) {
                            return $tenant->user->mobile;
                        });
                        $email = tenancy()->central(function ($tenant) {
                        ...
                        $info[0]=$phone;
                        $info[1]=$email;
						// $contentArr->addon_settings = json_encode($page_contact);
